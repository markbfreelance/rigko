...
export default function SuggestedBuildsHome() {
  return (
    <section className="w-full bg-black py-24 px-4 md:px-12 relative">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="reveal flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16 px-4">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight">FEATURED_BUILDS</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {builds.map((build, idx) => (
            <motion.div
              key={idx}
              whileHover={{ y: -10 }}
              className="reveal flex flex-col bg-white/[0.03] border border-white/5 rounded-3xl p-8 hover:bg-white/[0.06] transition-all group overflow-hidden relative"
            >
              {/* Card Background Decoration */}
              <div className="absolute top-0 right-0 p-4 font-mono text-[120px] leading-none text-white opacity-[0.02] select-none pointer-events-none group-hover:opacity-[0.05] transition-opacity">
                0{idx + 1}
              </div>
              ...
              <div className="flex justify-between items-start mb-12">
                ...
                <div className="text-2xl font-bold text-white tracking-tight">{build.price}</div>
              </div>

              <h3 className="text-xl font-bold text-white mb-8 group-hover:text-[#c2000b] transition-colors">{build.title}</h3>
              
              <div className="space-y-4 mb-10 pt-6 border-t border-white/5 font-mono">
                {build.specs.map((spec, sidx) => (
                  <div key={sidx} className="flex items-center justify-between">
                    ...
                    <div className="text-white text-sm">{build.specs[sidx].value}</div>
                  </div>
                ))}
              </div>

              <button className="w-full bg-white/5 text-white py-4 rounded-2xl text-sm font-bold hover:bg-[#c2000b] transition-all border border-white/10 group-hover:border-[#c2000b]/50">
                CONFIGURE_ASSETS
              </button>
            </motion.div>
          ))}
