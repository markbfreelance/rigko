...
export default function BuildPage() {
  ...
  const toggleCategory = (id: string) => setExpandedCats(prev => prev.includes(id) ? prev.filter(c => c !== id) : [...prev, id]);
                        ...
                        <div key={cat.id} className="w-full flex flex-col rounded-xl border border-black/5 dark:border-white/5 chassis-steel backdrop-blur-md transition-all overflow-hidden">
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-colors ${selectedPart ? "bg-[#c2000b]/10 border-[#c2000b]/30 text-[#c2000b]" : "bg-black/10 dark:bg-white/5 border-white/5 text-gray-400"
                                }`}>
                              ...
                              >
                                <div className="p-4 flex items-center justify-between gap-4">
                                  ...
                                  <button
                                    ...
                                    className={`px-6 py-2 rounded-full text-[9px] font-black uppercase tracking-tighter transition-all ${selectedPart
                    ...
                    <div className="relative z-10 grid grid-cols-3 gap-y-8 gap-x-4">
                      {PERIPHERAL_CATEGORIES.map((pcat) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`relative shrink-0 w-12 h-12 min-w-[48px] min-h-[48px] rounded-xl flex items-center justify-center border-2 transition-all shadow-md group/iconbox ${sel
                              ...
                              {/* Default Icon */}
                              <Icon icon={pcat.icon} className="text-xl group-hover:hidden" />

                              {/* Hover Icon (+ or x) */}
                              <Icon
                                icon={isActive ? "solar:close-circle-bold" : "solar:add-circle-bold"}
                            ...
                            >
                              <div className="pt-2">
                                <div className="w-full flex items-start gap-2 transition-all group/row">
                                  ...
                                  <div className={`relative shrink-0 w-12 h-12 min-w-[48px] min-h-[48px] flex flex-col items-center justify-center rounded-xl border-2 transition-all ${isActive
                                    ...
                                    <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-1 h-3 bg-gray-400 dark:bg-gray-600 rounded-full"></div>
                                    <Icon icon={pcat.icon} className={`text-xl transition-all ${sel ? "text-[#c2000b]" : isActive ? "text-[#c2000b]" : "text-gray-500"
                                      }`} />
                                  </div>

                                  {/* Block 2: The Details Container (2 Pins) */}
                                  <div className={`relative flex-1 flex flex-col rounded-xl border-2 transition-all ${isActive
                                    ? "bg-white dark:bg-[#1a1a1a] border-[#c2000b] shadow-[0_4px_12px_rgba(194,0,11,0.1)]"
                                    : "bg-white dark:bg-[#1a1a1a] border-[#e5e5e5] dark:border-[#222222] group-hover/row:border-[#c2000b]/30"
                                    }`}>
                                    ...
                                    {/* Header Action */}
                                    <button onClick={() => toggleCategory(pcat.id)} className="w-full flex items-center justify-between p-3 relative z-10">
                                      <span className={`text-[11px] font-bold uppercase tracking-tighter ${isActive ? "text-[#c2000b]" : "text-black dark:text-white"}`}>
                                        {pcat.name}
                                      </span>
                                      <div className="flex items-center gap-3">
                                        {sel && (
                                          <span className={`text-[9px] font-black uppercase truncate max-w-[120px] ${isActive ? "text-[#c2000b]" : "text-gray-500"}`}>{sel.name}</span>
                                        )}
                                        <Icon icon="solar:alt-arrow-down-linear" className={`text-gray-400 transition-transform duration-300 ${expandedCats.includes(pcat.id) ? "rotate-180 text-[#c2000b]" : "group-hover/row:text-[#c2000b]"}`} />
                                      </div>
                                    </button>
                                              ...
                                              <div className="flex flex-col gap-2">
                                                <div className="flex flex-wrap gap-x-3 gap-y-1">
                                                  ...
                                                  <div className="text-[8px] font-mono text-gray-500 uppercase tracking-widest leading-tight flex items-center">
                                                    {pcat.id === "monitor" && `${sel.size}" // ${sel.resolution} // ${sel.refresh_rate}Hz // ${sel.panel}`}
                                                    {pcat.id === "keyboard" && `${sel.type} // ${sel.switches} // ${sel.layout}`}
                                                    {pcat.id === "mouse" && `${sel.dpi} DPI // ${sel.weight}g // ${sel.wireless ? 'Wireless' : 'Wired'}`}
                                                    {pcat.id === "headset" && `${sel.connection} // ${sel.surround}`}
                                                    {pcat.id === "speaker" && `${sel.power} // ${sel.connection}`}
                                                    {pcat.id === "webcam" && `${sel.resolution} // ${sel.fps} fps`}
                                                  </div>
                                                </div>
                                                <div className="flex items-center justify-between mt-1">
                                                  <span className="text-[10px] font-black text-[#c2000b]">₱{sel.price.toLocaleString()}</span>
                                                  <button
                                                    onClick={(e) => {
                                                      e.stopPropagation();
                      ...
                      .map((part, idx) => {
                        ...
                        return (
                          <motion.div
                            ...
                            className={`group relative p-4 rounded-2xl border transition-all ${isSelected
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center justify-between mt-4">
                                  ...
                                  <button
                                    ...
                                    disabled={!compatible}
                                    className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-tighter shadow-lg transition-all ${isSelected
            ...
            <div className="sticky top-28 space-y-2">
              ...
              {CATEGORIES.map(cat => (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all border group ${activeCategory === cat.id
                  ...
                  {PERIPHERAL_CATEGORIES.map((pcat) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex flex-col items-center gap-2 group relative"
                      >
                        <div className={`relative shrink-0 w-11 h-11 min-w-[44px] min-h-[44px] rounded-xl flex items-center justify-center border-2 transition-all shadow-md group/iconbox ${sel
                          ? "bg-[#c2000b] border-[#ff4d4d] text-white shadow-[0_0_15px_rgba(194,0,11,0.3)]"
                          : isActive
                            ? "bg-white dark:bg-[#1a1a1a] border-[#c2000b] text-[#c2000b]"
                            : "bg-white dark:bg-[#1a1a1a] border-[#e5e5e5] dark:border-[#222222] text-gray-500 group-hover:border-[#c2000b]/30"
                      ...
                      return (
                        ...
                        >
                          <div className="pt-2">
                            <div className="w-full flex items-start gap-2 transition-all group/row">
                              {/* Block 1: The Icon (1 Pin) */}
                              <div className={`relative shrink-0 w-11 h-11 min-w-[44px] min-h-[44px] flex flex-col items-center justify-center rounded-xl border-2 transition-all ${isActive
                                ? "bg-white dark:bg-[#1a1a1a] border-[#c2000b] shadow-[0_4px_12px_rgba(194,0,11,0.1)]"
                                : "bg-white dark:bg-[#1a1a1a] border-[#e5e5e5] dark:border-[#222222] group-hover/row:border-[#c2000b]/30"
                                }`}>
                                <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-1 h-3 bg-gray-400 dark:bg-gray-600 rounded-full"></div>
                              ...
                              <div className={`relative flex-1 flex flex-col rounded-xl border-2 transition-all ${isActive
                                ...
                                {/* Header Action */}
                                <button onClick={() => toggleCategory(pcat.id)} className="w-full flex items-center justify-between p-2.5 relative z-10">
                                  <span className={`text-[11px] font-bold uppercase tracking-tighter ${isActive ? "text-[#c2000b]" : "text-black dark:text-white"}`}>
                                    {pcat.name}
                                  </span>
                                  <div className="flex items-center gap-3">
                                    {sel && (
                                      <span className={`text-[9px] font-black uppercase truncate max-w-[100px] ${isActive ? "text-[#c2000b]" : "text-gray-500"}`}>{sel.name}</span>
                                    )}
                                    <Icon icon="solar:alt-arrow-down-linear" className={`text-gray-400 transition-transform duration-300 ${expandedCats.includes(pcat.id) ? "rotate-180 text-[#c2000b]" : "group-hover/row:text-[#c2000b]"}`} />
                                  </div>
                                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              <AnimatePresence>
                {((PARTS[activeCategory as keyof typeof PARTS] || [])
                  ...
                  .map((part, idx) => {
                    const { compatible, reason, warning } = checkCompatibility(activeCategory, part);
                    const isSelected = selectedParts[activeCategory]?.id === part.id || selectedPeripherals[activeCategory]?.id === part.id;
                    ...
                    return (
                      <motion.div
                        ...
                        transition={{ delay: idx * 0.05 }}
                        onClick={() => handleSelect(activeCategory, part)}
                        className={`cursor-pointer group relative p-4 md:p-6 rounded-3xl border-2 transition-all ${isSelected
                      ...
                      >
                        ...
                        {(!compatible || (warning && !isSelected)) && (
                          <div className="absolute inset-x-0 -top-3 z-20 flex justify-center">
                            <div className={`backdrop-blur-md text-white text-[8px] md:text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border flex items-center gap-2 shadow-xl ${!compatible ? "bg-[#c2000b] border-[#c2000b]" : "bg-yellow-600/90 border-yellow-400/50"
                              }`}>
                              <Icon icon={!compatible ? "solar:danger-bold" : "solar:shield-warning-bold"} className="text-white text-base" />
                              {reason}
                            </div>
                          </div>
                        )}
            ...
            >
              ...
              <div className="relative z-10 p-8 pl-12 h-full flex flex-col pointer-events-none">
                ...
                <div className="relative z-10 shrink-0 mt-2 mb-6 overflow-y-auto max-h-[140px] pr-2 custom-scrollbar border-t border-black/5 dark:border-white/5 pt-4 pointer-events-auto">
                  <div className="space-y-2">
                    {[...Object.entries(selectedParts), ...Object.entries(selectedPeripherals)].map(([catId, part]) => {
                      if (!part) return null;
                      const category = CATEGORIES.find(c => c.id === catId) || PERIPHERAL_CATEGORIES.find(c => c.id === catId);
                      return (
                    ...
                    {Object.values(selectedParts).filter(Boolean).length === 0 && Object.values(selectedPeripherals).filter(Boolean).length === 0 && (
      ...
      <div className="fixed bottom-0 left-0 w-full z-150 chassis-steel backdrop-blur-md border-t border-black/10 dark:border-white/10 px-8 py-4">
        ...
        <div className="max-w-[1440px] mx-auto flex items-center justify-between relative z-10 gap-x-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className={`px-6 md:px-12 py-2.5 md:py-3 rounded-full font-black uppercase text-[10px] md:text-xs tracking-widest transition-all ${totalPrice > 0
